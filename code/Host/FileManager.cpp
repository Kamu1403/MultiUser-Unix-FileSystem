...
void FileManager::Open() {
    ...
    }if (u.i_uid == pINode->i_uid) {
        if(u.arg[1]& File::FREAD)
            if(!(pINode ->i_mode&INode::IREAD))
                u.u_error = User::U_EACCES;
        if (u.arg[1] & File::FWRITE)
    ...
    {
        if (u.arg[1] & File::FREAD)
            if (!(pINode->i_mode & (INode::IREAD >> 6)))
                u.u_error = User::U_EACCES;
        if (u.arg[1] & File::FWRITE)
            if (!(pINode->i_mode & (INode::IWRITE >> 6)))
                u.u_error = User::U_EACCES;
    }
...
void FileManager::Open1(INode* pINode, int mode, int trf) {
    ...
    /* 分配打开文件控制块File结构 */
    File* pFile = this->openFileTable->FAlloc((trf==0)?(unsigned int)pINode:0);
    if (NULL == pFile) {
        if (u.u_error == User::U_EACCES) {
            osstream << "File locked by other user."<<endl;
        }
        else if(u.u_error = User::U_ENFILE){
            this->inodeTable->IPut(pINode);
        }
        return;
    }
...
void FileManager::WriteDir(INode* pINode) {
    User& u = g_User;
    //osstream << "i_number=" << pINode->i_number << endl;
    /* 设置目录项中INode编号部分 */
...
void FileManager::Seek() {
    ...
    case 2:
        ...
        break;
    default:
        osstream << " origin " << u.arg[2] << " is undefined ! \n";
        break;
    }
}
...
void FileManager::Ls() {
    ...
    while (u.IOParam.count) {
        ...
        {
            INode* tmpINode = this->inodeTable->IGet(u.dent.m_ino);

            if (NULL == tmpINode) {
                printf("目录：%s，索引错误。", u.dent.name);
            }
            else
            {
                u.ls += Stat(tmpINode);
...
void FileManager::Chmod(unsigned int mode) {
    User& u = g_User;
    INode* pINode;
    pINode = this->NameI(FileManager::OPEN);
    if (NULL == pINode) {
