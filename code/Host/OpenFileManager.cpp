...
File* OpenFileTable::FAlloc(unsigned int inodeAddr) {
	User& u = g_User;
    ...
    if (inodeAddr) {
        for (int i = 0; i < OpenFileTable::MAX_FILES; ++i) {
            if ((unsigned int)this->m_File[i].inode == inodeAddr) {
                if (!(this->m_File[i].lock_uid==0||this->m_File[i].lock_uid==u.i_uid)){
                    u.u_error = User::U_EACCES;
                    return NULL;
                }
                else{
                    this->m_File[i].lock_uid = u.i_uid;
...
INode* INodeTable::IGet(int inumber) {
    ...
    pINode = GetFreeINode();
    if (NULL == pINode) {
        osstream << "INode Table Overflow !" << endl;
